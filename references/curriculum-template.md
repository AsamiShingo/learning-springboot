...
| 6 | enum(状態・種別の型安全な表現) | ロールや種別のような固定値を、生の文字列比較ではなく`enum`で表現している | Step9(レイヤードアーキテクチャ)、既存コードに生文字列でのロール比較(例: `"ADMIN".equals(user.getRole())`)が残っていればその見直し。題材にロール・種別に相当する概念が無い場合は、「どこで使うと有効そうか」を口頭で説明できれば「会話で確認済み」でよい |
