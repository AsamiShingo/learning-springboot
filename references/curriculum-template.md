...
Step13(セッション認証)/ Step14(REST API)/ Step24(外部API連携)/
Step15(Security入門)/ Step16(認可仕上げ)/ Step17(ロギング・Interceptor・Filter)/
Step18(運用仕上げ・任意)/ Step19(JS基礎)/ Step20(CSSレイアウト)/
Step21(クライアントバリデーション)/ Step22(jQuery)/ Step23(Ajax+CSRF)/ 発展メニュー

(Step24は後から追加されたため番号は末尾寄りだが、目次上はStep14の直後
— 実際の学習順 — に記載している。番号と目次上の並び順が異なる唯一の例)

## 修了基準

このカリキュラムにおける「修了」は、**Step0〜Step23およびStep24の全カリキュラムが
完了していること**(Step0チェックリストは16項目すべて`未実証`以外の状態になっている、
Step1〜24は全て「完了」)を基本ラインとする。発展メニューは修了の必須条件には含めない。
...
| 14 | 継承・ポリモーフィズム・抽象クラス | 共通処理を親クラス/抽象クラスに切り出している、または同じinterface/親クラスに対して異なる実装を差し替えて使う設計がある | Step9(レイヤードアーキテクチャ)。この規模のアプリでは継承階層が自然には出てこないことも多いため、無ければ「どこで使うと有効そうか」「なぜここでは使わなかったか」を口頭で説明できるかの確認(「会話で確認済み」ステータス)でよい。今後も出てくる見込みが無いと判断した場合は「対象外」でもよい |
...
- 各項目の状態は「未実証 / 会話で確認済み / コードで実証済み / 対象外」の4段階。
  `.claude/state/learning-springboot/PROGRESS.md`にStep0専用のサブテーブルとして持つ。
  項目7c(ファイルI/O)・項目14(継承階層)のように、この規模のアプリではそもそも
  出現しえないと判断した場合は「対象外」を、コードには現れていないが口頭で正しく
  説明できることを確認できた場合は「会話で確認済み」を用いる。
...
- 前提Step: Step0
- 完了条件: 以下を**モード3のレビュー時に実際に示せる**こと(口頭確認のみで完了としない)。
  1. 既存コードにわざと文法エラー(タイポ等)を入れ、IDE上に赤波線/エラー表示が出ることを見せる。
  2. 何らかのオブジェクトに対してメソッド呼び出しの補完候補が表示されることを見せる。
  3. 任意の行にブレークポイントを置き、IDEのデバッグ実行で実際に停止させ、
     その時点の変数の値をインスペクトできることを見せる。
...
## Step24: 外部API連携(他システムのAPI呼び出し)
- 番号についての注記: このStepは後から追加されたため番号が末尾寄り(24)になっているが、
  **学習順としてはStep14の直後(Step15の前)に行うことを推奨する**。前提Step関係が
  実際の学習順を表すので、番号の大小は無視してよい(既存Stepの番号を振り直さない
  という運用ルールのため、新規Stepは常に末尾の番号で追加される。詳細は`SKILL.md`の
  初期化ロジック参照)。
...
- 前提Step: Step14(自分がAPIを提供する経験があると、呼び出す側の視点との対比が理解しやすい)
- 完了条件:
...
- 前提Step: Step19
- 完了条件: Step19で書いたDOM操作の一部をjQueryで書き換え、コード量・書き方の
