<!-- template-version: 2026-08-22.16 -->
...
**idを付ける対象は「学習内容を持つセクション」全般**であり、Stepの見出しに限らない
(例:「アプリの題材決め」「発展メニュー」「React編」「フロントエンド編」の各見出し)。
これらもテンプレート更新時の差分検出対象になる。一方、`## 目次`や`## 修了基準`の
ような、他セクションの内容をまとめて索引・要約するだけの見出しにはidを付けない
(単独で差分検出する意味が薄いため)。
...
- **項目7のように実証条件が(a)(b)(c)に分かれている項目でも、状態は項目に1つだけ**。
  条件ごとに判断した上で、**1つでも`未実証`の条件が残っていれば項目全体は`未実証`**とし、
  条件ごとの現状は実証箇所欄に並べて書く(例: 上記の「項目7c(ファイルI/O)を対象外に
  する」という判断も、(a)(b)が済んでいて初めて項目全体の状態に反映される)。
  一部だけ実証された項目を「コードで実証済み」にすると、修了判定がそれを対応済みとして
  数えてしまう。詳細は`SKILL.md`の「PROGRESS.mdのフォーマット」節を正とする。
...
**これは尋ねるときだけの決まりではなく、題材決めの対話が終わるまで一貫して適用する**。
題材を確定して「この題材で大丈夫です」と伝える場面や、補強を提案する場面でも同じ。
「関連しあうデータが3種類あるので**Step10のN+1問題**で困りません」のように、
確定の理由づけとして未習の概念名やStep名・Step番号を持ち出さない(学習者にとっては
判断も検証もできない情報であり、この段階では納得の材料にならない)。理由を伝えたい
場合も日常の言葉に置き換える。例:「関連しあうデータが複数あるので、後半で扱う
少し実務寄りの内容まで、この題材のまま進められそうです」「絞り込みの対象になる項目が
あるので、一覧を扱う回でそのまま使えます」。どのStepで何を学ぶかは、実際にその
Stepに着手するとき(モード1)に説明すれば足りる。
