...
| 6 | enum(状態・種別の型安全な表現) | ロールや種別のような固定値を、生の文字列比較ではなく`enum`で表現している | Step9(レイヤードアーキテクチャ)、既存コードに生文字列でのロール比較(例: `"ADMIN".equals(user.getRole())`)が残っていればその見直し。「アプリの題材決め」で利用者の役割が2種類以上あることを確認しているため、原則としてロール・種別に相当する概念が題材に存在するはずであり、「対象外」「会話で確認済み」への緩和は原則使わない |
