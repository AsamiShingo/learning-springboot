<!-- template-version: 2026-09-23.05 -->
...
Step23(運用仕上げ・任意)/ Step24(JS基礎・任意)/ Step25(jQuery・任意)/
Step26(Ajax+CSRF・任意)/ Step27(クライアントバリデーション・任意)/
Step28(React基礎・任意)/ Step29(コンポーネント/props・任意)/
Step30(CSSレイアウト・任意)/ Step31(State/イベント・任意)/ Step32(データ取得/CORS・任意)/
Step33(React統合・任意)/ Step34(React Router・任意)/
...
graph LR
  ...
  S22 -.-> S23
  S22 -.-> S24[Step24 JS基礎]
  S24 --> S25[Step25 jQuery]
  S18 --> S26[Step26 Ajax+CSRF]
  S20 --> S26
  S25 -.-> S26
  S26 --> S27[Step27 クライアントバリデーション]
  S25 -.-> S28[Step28 React基礎]
  S28 --> S29[Step29 コンポーネント/props]
  S29 -.-> S30[Step30 CSSレイアウト]
  S29 --> S31[Step31 State/イベント]
...
- 概念: `@Query`によるJPQL、ネイティブクエリ(`nativeQuery = true`)との使い分け、
  ...
  効かない点に注意)による発行SQLの確認。余力があれば、インデックスと`EXPLAIN`による
  実行計画の確認まで踏み込んでもよい。
...
- 完了条件:
  ...
  - 一覧検索で実際に発行されるSQLをログで確認している。
- 発展(任意): 検索条件のカラムにインデックスを張った場合と張らない場合で、実行計画
  (`EXPLAIN`)がどう変わるかを見てみるとよい。ただしH2の`EXPLAIN`は使用インデックスの
  有無が分かる程度の簡素な出力で、実行コストや推定行数は出ない。実行計画を読む練習として
  取り組むならPostgreSQLを選んでいる場合に向く。
...
## フロントエンド編(Step24〜27、任意/発展、バックエンド完了後) <!-- id: frontend-section-intro -->

**このフロントエンド編(Step24〜27)は全体が任意/発展Stepであり、修了の必須条件には
...
- 完了条件: 素のJavaScriptだけで、イベントに応じて画面の一部を書き換えられる(どのイベントを使ったかは問わない)。
  **ユーザー入力(またはDBの登録データ)由来の文字列をDOMに挿入する箇所について、
  `innerHTML`でエスケープ無しに挿入していないか確認し、`textContent`を使うか
  意図的にエスケープしている**。なぜそうすべきか(XSS)を説明できる。

## Step25: jQuery入門(任意/発展) <!-- id: jquery-basics -->
- 目的: 素のJavaScriptで書いていたDOM操作・イベント処理をjQueryで書き直し、
  何が簡潔になるかを比較する(深追いはしない)。
- 概念: セレクタによる要素の取得、イベントへの反応、画面の状態変更(値・クラス・
  表示・要素の追加削除など)。あわせて、切り替え先として自分で書くボタン程度の
  CSS装飾(レイアウトはStep30で扱う)。
- 前提Step: Step24
- 完了条件: Step24で書いたDOM操作の一部をjQueryで書き換え、違いを説明できる。
  セレクタで要素を取得し、イベントに反応して、画面の状態を変える処理がjQueryで
  実現できている。どのメソッド・どのイベントを使ったかは問わない。
- 補足: 動的に追加した要素にもイベントを効かせる「イベント委譲」は、ここでは
  そういう書き方があると知っておく程度で十分。

## Step26: Ajax通信とCSRF連携(任意/発展) <!-- id: ajax-csrf -->
...
- 前提Step: Step18(REST API)、Step20(Security/CSRF)、Step25
- 完了条件: 画面遷移無しでStep18のREST APIにPOSTでき、CSRF保護が有効なままリクエストが成功する。ブラウザの開発者ツールでリクエスト/レスポンスを確認できる。

## Step27: クライアントサイドバリデーション(任意/発展) <!-- id: client-validation -->
- 目的: Step6のサーバーサイドバリデーションと対比しながら、即時フィードバック用のクライアント側
  チェックを実装する。Step26で作ったAjax送信の手前にチェックを挟み、通らなければ通信自体を
  行わない形にする。
- 概念: HTML5バリデーション属性(`required`等)、jQueryによる入力チェック
  (`.on('submit')`でのフック、`.val()`での値取得、`.addClass()`/`.text()`でのエラー表示)、
  送信前チェックによる無駄な通信の抑止、サーバー側検証との役割分担。
- 前提Step: Step26(jQueryの操作一式と、チェック対象になるAjax送信があること)
- 完了条件: 明らかな入力ミス(未入力・形式違反)は送信前にクライアント側で気づける。
  チェックに通らない場合はAjaxリクエストが送信されないことを開発者ツールで確認している。
  最終チェックはサーバー側(Step6)に残っている。
- 補足: Step25で一通り使ったjQueryの操作を、実際の機能として組み合わせる回でもある。

## React編(Step28〜36、任意/発展。jQuery完了後、または相当の知識がある場合は直接) <!-- id: react-section-intro -->

これまでのStep24〜27はjQuery/素のJavaScriptによるフロントエンド実装だったが、
...
含めない**。jQuery(Step25)までの実装で「Web開発の一通りの基礎」は完了しているため、
Reactへの移行は「実務でSPA構成に触れる機会を前倒しで作りたい」学習者向けの
追加メニューという位置づけ。

**入口は2ルートある**:
- **jQuery経由**: Step24〜27を完了した上でReactに取り組む。jQueryのDOM操作・Ajax・
  CSRFの経験と対比しながら学べるため、初めてフロントエンドに触れる学習者にはこちらを推奨する。
- **直接React**: すでにjQueryやDOM操作・イベント処理の基礎(他プロジェクトでの経験等)が
  ある学習者は、Step24〜27を経由せずStep28から直接始めてよい。この場合、Step26で
...
- 前提Step: なし(依存関係マップの`S25 -.-> S28`(点線)は「jQuery経由」の推奨経路であり
  必須ではない)。DOM操作/イベント処理の基礎がある学習者はStep24〜27を経由せず直接始めてよい。
  未経験なら先にStep24〜27を通る方が理解が早い。
...
## Step30: CSSレイアウト(任意/発展) <!-- id: css-layout -->
- 目的: 装飾ではなくレイアウト崩れを直せるレベルのCSSを身につける(画面構造の共通化はStep5、
  ボタン程度の装飾はStep25で対応済み)。Step29で分割したコンポーネントを実際に配置しながら進める。
- 概念: Flexbox/Grid、メディアクエリ(レスポンシブ)。
- 前提Step: Step29(推奨順のみ——CSS自体はReactに技術的に依存しないが、配置する対象と
  してコンポーネントが分割済みだと学びやすい)
- 完了条件: 主要画面がFlexbox/Gridで崩れずに表示される。画面幅を変えたときの
  崩れをメディアクエリで1箇所以上直している。
...
- 目的: Step28〜32で身につけた要素(JSX、コンポーネント分割、state、データ取得)を
...
- 概念: 既存Thymeleaf画面とReact画面の共存(画面単位の切り替えでよい)、既存のCSS
  (Step30)との整合、propsのバケツリレー(複数階層のコンポーネントを経由してstateを
...
- 目的: 表示だけでなく、Reactからのフォーム送信(作成/更新)をSpring SecurityのCSRF
  保護下で実現し、Step26で学んだAjax+CSRFの知識をReact側でも再現する。
- 概念: POST/PUTリクエストの送信、CSRFトークンの取得・送信(Step26と同様の方式をReact側に
  適用)、送信後の再取得と画面表示の更新。
- 前提Step: Step34、Step26(推奨順のみ——Ajax+CSRFの知識があると理解が早い。
  Step26自体を経由していない学習者は、この時点でCSRFトークンの扱いを新たに
  学ぶことになる旨を伝える)
