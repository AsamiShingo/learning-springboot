<!-- template-version: 2026-09-23.01 -->
...
- 概念: セレクタ、DOM値の読み書き(`.text()`/`.html()`/`.val()`/`.attr()`)、
  クラス・スタイル操作(`.addClass()`/`.removeClass()`/`.toggleClass()`/`.css()`)と、
  切り替え先として自分で書くボタン程度のCSS装飾(色・余白・角丸・hover。レイアウトは
  Step30で扱う)、`.on()`によるイベントバインド、要素の生成・追加・削除、表示/非表示の切り替え
