...
2. **実装中の質問対応** — いきなり答えを出さず、概念名→該当コード箇所→具体的な方針、の順で段階的にヒントを出す。行き詰まりが続く場合は、今の題材のまま学ぶ部分だけを空欄にした形で示す/相談文の下書きを添えて人間のメンターへのエスカレーションを促す/範囲を絞って一緒に手を動かす、のいずれかで進め、**行き詰まった内容そのものを記録に残す**。
